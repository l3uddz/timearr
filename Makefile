...
${BUILD_PATH}/${CMD}: ${GO_FILES} go.sum
	...
	CGO_ENABLED=0 go build \
		-mod vendor \
		-trimpath \
		-ldflags "-s -w -X github.com/l3uddz/timearr/runtime.Version=${VERSION} -X github.com/l3uddz/timearr/runtime.GitCommit=${GIT_COMMIT} -X github.com/l3uddz/timearr/runtime.Timestamp=${TIMESTAMP}" \
		-o ${BUILD_PATH}/${CMD} \
		.
